...
export default function VenueOwnerLanding() {
  ...
  const renderActionButton = (size = "normal") => {
    ...
    if (hasExistingRole) {
      return (
        <div className={`flex flex-col items-center gap-3 ${isHero ? 'w-full max-w-md' : 'w-full max-w-sm'}`}>
          <div className={`w-full bg-white/5 border border-[#BFF367]/20 backdrop-blur-sm rounded-[10px] ${isHero ? 'p-5 md:p-6' : 'p-4 md:p-5'} text-center`}>
            ...
            <button
              ...
              className={`bg-[#BFF367] text-black font-black uppercase tracking-widest rounded-[8px] hover:brightness-110 transition-all flex items-center justify-center gap-2 mx-auto ${isHero ? 'px-6 py-2.5 text-sm' : 'px-5 py-2 text-xs'}`}
            >
    ...
    if (hasPendingApplication) {
      return (
        <div className={`flex flex-col items-center gap-3 ${isHero ? 'w-full max-w-md' : 'w-full max-w-sm'}`}>
          <div className={`w-full bg-white/5 border border-amber-500/20 backdrop-blur-sm rounded-[10px] ${isHero ? 'p-5 md:p-6' : 'p-4 md:p-5'} text-center`}>
            <div className="flex items-center justify-center gap-2 mb-2">
            ...
            <p className="text-white/60 text-xs leading-relaxed">You have applied for <span className="text-amber-400 font-bold">{user?.applicationRole || 'Venue Owner'}</span>. We are verifying your application. You will be notified once approved.</p>
          </div>
        </div>
      );
    }
    ...
    return (
      <button onClick={handleRegisterClick} className={`bg-[#BFF367] text-black font-black rounded-[8px] hover:scale-105 transition-transform shadow-[0_0_20px_rgba(191,243,103,0.25)] ${isHero ? 'px-10 py-4 text-lg' : 'px-12 py-5 text-lg'}`}>
        Register Your Venue
      </button>
    );
  };
