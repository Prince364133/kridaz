        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
          ...
          <div className="lg:col-span-2 p-6 md:p-10 bg-[#0A0A0A] rounded-[2.5rem] border border-white/5 relative group transition-all hover:border-[#84CC16]/20">
            <div className="flex justify-between items-center mb-8 md:mb-12">
              <div>
                <h2 className="text-xl md:text-2xl font-black uppercase tracking-tight text-white">Student Progress</h2>
                <p className="text-[10px] font-bold text-gray-500 uppercase tracking-[0.2em] mt-1">Aggregate Achievement Metrics</p>
              </div>
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={studentProgress} margin={{ top: 20, right: 20, left: 0, bottom: 0 }}>
                    ...
                    <Tooltip 
                      ...
                      itemStyle={{color: '#84CC16', fontWeight: 'bold', textTransform: 'uppercase', fontSize: '10px'}}
                    />
                    <Area type="monotone" dataKey="value" stroke="#84CC16" strokeWidth={4} fill="url(#coachColor)" />
                  </AreaChart>
                </ResponsiveContainer>
              ) : (
                <div className="text-center space-y-3 opacity-30">
                  <Layout size={32} className="mx-auto" />
                  <p className="text-[10px] font-black uppercase tracking-[0.3em]">Telemetry Required</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="p-8 md:p-10 bg-[#0A0A0A] rounded-[2.5rem] border border-white/5 flex flex-col group hover:border-[#84CC16]/20 transition-all">
            <h2 className="text-xl md:text-2xl font-black uppercase tracking-tight text-white mb-8">Upcoming Sessions</h2>
            <div className="flex-1 space-y-5 overflow-y-auto max-h-[450px] pr-2 custom-scrollbar">
              {upcomingSessions.length > 0 ? (
                upcomingSessions.map((op, i) => (
                  <div key={i} className="p-5 bg-white/[0.02] border-l-4 border-[#84CC16] group/item hover:bg-white/[0.05] transition-all rounded-r-[1.5rem]">
                    <div className="flex justify-between items-start mb-2">
                      <span className="text-[10px] font-black text-gray-500 uppercase tracking-wider">{op.time}</span>
                      <span className="px-2 py-1 bg-[#84CC16]/10 text-[#84CC16] text-[8px] font-black uppercase tracking-widest rounded-md">{op.type}</span>
                    </div>
                    <h4 className="text-base font-bold text-white mb-1 uppercase tracking-tight">{op.name}</h4>
                ...
                <div className="h-full flex flex-col items-center justify-center text-center space-y-4 opacity-20 py-20">
                   ...
                   <span className="text-[10px] font-black uppercase tracking-[0.3em] leading-relaxed">No pending <br />assignments</span>
                </div>
              )}
            </div>
