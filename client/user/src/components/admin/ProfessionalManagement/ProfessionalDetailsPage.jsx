...
const ProfessionalDetailsPage = () => {
  ...
  if (!details) {
    return (
      <div className="min-h-screen bg-[#0a0a0a] flex flex-col items-center justify-center p-6 text-center space-y-4">
        ...
        <button 
          onClick={() => navigate(-1)}
          className="mt-6 px-6 py-2 bg-[#84CC16] text-black font-bold uppercase rounded-lg hover:bg-[#65a30d] transition-colors"
        >
          Go Back
        </button>
      </div>
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white p-6 lg:p-10 relative overflow-hidden">
      ...
      <div className="absolute top-[-10%] left-1/2 -translate-x-1/2 w-[80%] h-64 bg-[#84CC16]/10 blur-[150px] pointer-events-none"></div>
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="bg-[#111] border border-white/10 rounded-2xl shadow-2xl flex flex-col overflow-hidden">
          
          {/* Profile Header */}
          <div className="p-8 pb-0 flex flex-col sm:flex-row gap-8 items-start sm:items-end justify-between border-b border-white/10">
            <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6 sm:gap-8 pb-8 text-center sm:text-left">
              <div className="w-24 h-24 sm:w-32 sm:h-32 rounded-full bg-[#84CC16]/10 flex items-center justify-center text-[#84CC16] font-bold text-4xl sm:text-5xl uppercase border-2 border-[#84CC16]/30 shadow-[0_0_30px_rgba(132,204,22,0.15)] overflow-hidden shrink-0">
              ...
              <div className="mt-2 sm:mt-4">
                <div className="flex flex-col sm:flex-row items-center gap-3 sm:gap-4 mb-3">
                  ...
                  <span className="px-3 py-1 bg-[#84CC16]/20 text-[#84CC16] rounded-full text-xs sm:text-sm font-bold tracking-wider border border-[#84CC16]/20 uppercase">
                    ACTIVE {profile?.role}
                  </span>
                </div>
                <div className="flex flex-wrap justify-center sm:justify-start gap-4 text-sm text-gray-400">
                  <div className="flex items-center gap-1.5">
                    <Star size={16} className="text-[#84CC16]" />
            ...
            <div className="hidden lg:flex gap-6 pb-8 text-right">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-[#84CC16]">₹{profile?.price || 0}</p>
              </div>
            </div>
          </div>

          {/* Tabs */}
          <div className="flex overflow-x-auto border-b border-white/10 bg-[#111]/50 custom-scrollbar">
            {["overview", "finances", "matches"].map((tab) => (
              <button
                key={tab}
                onClick={() => setActiveTab(tab)}
                className={`px-8 py-5 text-sm font-bold uppercase tracking-wider transition-all duration-300 relative whitespace-nowrap ${
                  activeTab === tab ? "text-[#84CC16]" : "text-gray-500 hover:text-gray-300"
                }`}
              >
                {tab}
                {activeTab === tab && (
                  <div className="absolute bottom-0 left-0 w-full h-0.5 bg-[#84CC16] shadow-[0_0_10px_rgba(132,204,22,0.5)]"></div>
                )}
              </button>
            ))}
          </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-10">
                ...
                <div className="space-y-6">
                  <h3 className="text-xl font-bold text-white uppercase tracking-wider flex items-center gap-3">
                    <Mail className="text-[#84CC16]" size={24} /> Contact Details
                  </h3>
                ...
                <div className="space-y-6">
                  <h3 className="text-xl font-bold text-white uppercase tracking-wider flex items-center gap-3">
                    <Briefcase className="text-[#84CC16]" size={24} /> Professional Background
                  </h3>
                  <div className="bg-white/5 border border-white/10 rounded-xl p-6 space-y-5">
                    <div className="flex justify-between items-center border-b border-white/5 pb-4">
                    ...
                    <div className="flex justify-between items-center border-b border-white/5 pb-4">
                      ...
                      <span className="text-[#84CC16] font-bold text-lg">₹{profile?.price || 0}</span>
                    </div>
                ...
                <div className="space-y-6 lg:col-span-2 mt-4">
                  <h3 className="text-xl font-bold text-white uppercase tracking-wider flex items-center gap-3">
                    <Award className="text-[#84CC16]" size={24} /> Certifications
                  </h3>
                  {profile?.certifications?.length > 0 ? (
                    <div className="flex flex-wrap gap-4">
                      {profile.certifications.map((cert, idx) => (
                        <div key={idx} className="bg-[#1a1a1a] border border-[#84CC16]/30 text-gray-200 px-5 py-3 rounded-lg text-sm flex items-center gap-3 shadow-[0_4px_12px_rgba(0,0,0,0.5)]">
            ...
            {activeTab === "finances" && (
              <div className="space-y-10">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                  <div className="bg-gradient-to-br from-[#84CC16]/20 to-transparent border border-[#84CC16]/40 rounded-2xl p-6 relative overflow-hidden shadow-[0_0_30px_rgba(132,204,22,0.1)]">
                    <div className="absolute -right-4 -bottom-4 opacity-20 text-[#84CC16]"><IndianRupee size={100} /></div>
                    <p className="text-white/80 text-xs uppercase tracking-widest font-bold mb-3 relative z-10">Usable Balance</p>
                    <p className="text-4xl font-bold text-[#84CC16] relative z-10">₹{profile?.walletBalance?.toFixed(2) || "0.00"}</p>
                  </div>
                ...
                <div className="space-y-6">
                  <h3 className="text-xl font-bold text-white uppercase tracking-wider flex items-center gap-3">
                    <CreditCard className="text-[#84CC16]" size={24} /> Banking & KYC
                  </h3>
            ...
            {activeTab === "matches" && (
              <div className="space-y-6">
                <div className="flex justify-between items-center mb-4">
                  <h3 className="text-xl font-bold text-white uppercase tracking-wider flex items-center gap-3">
                    <Activity className="text-[#84CC16]" size={24} /> Match History
                  </h3>
                ...
                {matches?.length === 0 ? (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {matches?.map(match => (
                      <div key={match._id} className="bg-[#1a1a1a] border border-white/10 rounded-xl p-6 hover:border-[#84CC16]/50 hover:bg-white/5 transition-all duration-300 relative group">
                        {/* Status Indicator */}
                        ...
                        <div className="flex items-center gap-3 mb-4">
                          ...
                          <span className="text-[#84CC16] text-sm font-bold uppercase tracking-wider">
                            {match.gameType}
                          </span>
                        </div>
                        ...
                        <div className="flex items-end justify-between pt-4 border-t border-white/5">
                          ...
                          <div className="text-right">
                            ...
                            <div className="text-[#84CC16] font-bold text-xl">₹{match.umpireCost || 0}</div>
                          </div>
                        </div>
                      </div>
                    ))}
