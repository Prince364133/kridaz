      ...
      <div className="relative group">
        <div className="relative flex flex-row items-center bg-black/80 backdrop-blur-3xl border border-white/10 rounded-[8px] p-2 shadow-[0_30px_60px_-15px_rgba(0,0,0,0.8)] transition-all duration-500 hover:border-[#BFF367]/40 min-h-[60px] md:min-h-[70px]">
          ...
          <div className="flex-1 min-w-[80px] relative z-[100] border-r border-white/5" ref={sportDropdownRef}>
            ...
            >
              <Search className={`hidden sm:block mr-1 md:mr-2 shrink-0 w-3 h-3 md:w-3.5 md:h-3.5 transition-transform duration-500 ${showSportDropdown ? "scale-110 text-[#BFF367]" : "text-gray-500"}`} />
              ...
              <ChevronDown className={`hidden md:block ml-auto w-2.5 h-2.5 text-gray-600 transition-transform duration-500 ${showSportDropdown ? "rotate-180 text-[#BFF367]" : ""}`} />
            </button>
            {showSportDropdown && (
              <div className="absolute top-full mt-3 left-0 w-64 bg-[#0a0a0a] border border-white/10 rounded-[8px] overflow-hidden shadow-[0_20px_50px_rgba(0,0,0,0.9)] z-[110]">
                <div className="p-2 max-h-[250px] overflow-y-auto grid grid-cols-1 gap-1">
                  ...
                  {SPORTS_LIST.map((s) => (
                    <button key={s} onClick={() => { setSport(s); setShowSportDropdown(false); }} className={`w-full text-left px-4 py-2 text-[10px] font-bold rounded-[8px] uppercase transition-colors ${sport === s ? "bg-[#BFF367] text-black" : "text-gray-400 hover:bg-white/5"}`}>{s}</button>
                  ))}
                </div>
              </div>
            )}
          ...
          <div className="flex-[1.5] min-w-[150px] relative z-[90]" ref={locationRef}>
            <div className="flex items-center justify-center w-full h-full px-2 md:px-5 py-2 md:py-3">
              <MapPin className={`hidden sm:block mr-1 md:mr-2 shrink-0 w-3.5 h-3.5 md:w-4 md:h-4 transition-colors duration-500 ${locationInput ? "text-[#BFF367]" : "text-gray-500"}`} />
