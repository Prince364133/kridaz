          ...
          <div className="space-y-6 max-w-xl">
            <Link to="/" className="group flex items-center gap-4">
              ...
              <div className="border-l border-white/20 pl-4 h-10 flex flex-col justify-center">
                <span className="block text-[10px] font-semibold text-[#BFF367] tracking-wider uppercase leading-none mb-1">Venue Owner Portal</span>
            ...
            <div className="flex gap-4">
              {[Facebook, Linkedin, Instagram].map((Icon, i) => (
                <Link key={i} to="#" className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center text-white/40 hover:text-[#BFF367] hover:bg-white/10 transition-all">
                  <Icon size={18} />
                </Link>
              ))}
            </div>
            ...
            <div className="space-y-6">
              ...
              <ul className="space-y-3">
                {["Venue Dashboard", "Booking Manager", "Revenue Reports", "Facility Setup"].map((link) => (
                  <li key={link}>
                    <Link to="#" className="text-white/40 hover:text-[#BFF367] transition-colors text-sm">
                      {link}
                    </Link>
                  </li>
                ))}
            ...
            <div className="space-y-6">
              ...
              <ul className="space-y-3">
                {[
                  ...
                  <li key={link.name}>
                    <Link to={link.path} className="text-white/40 hover:text-[#BFF367] transition-colors text-sm">
                      {link.name}
                    </Link>
                  </li>
                ))}
