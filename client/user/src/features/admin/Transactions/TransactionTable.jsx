        ...
        <thead>
          <tr className="bg-white/5">
            ...
            <th className="p-4 text-xs font-bold uppercase tracking-widest text-gray-500 border-b border-white/5">
              <button
                className="flex items-center group transition-colors hover:text-[#BFF367]"
                onClick={() => onSort("createdAt")}
              >
                Date
                {sortField === "createdAt" &&
                  (sortDirection === "asc" ? (
                    <ChevronUp className="ml-2 h-3 w-3 text-[#BFF367]" />
                  ) : (
                    <ChevronDown className="ml-2 h-3 w-3 text-[#BFF367]" />
                  ))}
              </button>
            </th>
            ...
            <th className="p-4 text-xs font-bold uppercase tracking-widest text-gray-500 border-b border-white/5 text-right">
              <button
                className="flex items-center justify-end w-full group transition-colors hover:text-[#BFF367]"
            ...
            <tr key={transaction._id} className="group hover:bg-white/[0.02] transition-colors">
              <td className="p-4">
                <div className="flex items-center gap-3">
                  <div className="p-1 rounded-full bg-white/5 group-hover:bg-[#BFF367]/20 transition-colors">
              ...
              <td className="p-4">
                 <span className="text-xs font-bold uppercase tracking-widest text-[#BFF367]">{transaction.turf?.name || "Unknown"}</span>
              </td>
              ...
              <td className="p-4 text-right">
                 <span className="font-bold text-sm text-[#BFF367] tracking-tight">Rs {transaction.totalPrice}</span>
              </td>
            </tr>
          ))}
        </tbody>
