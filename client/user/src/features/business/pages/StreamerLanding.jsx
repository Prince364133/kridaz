...
import { ArrowRight, Video, Youtube, Globe, CheckCircle, Smartphone, ShieldCheck, X, User, Landmark, QrCode, Upload, CheckCircle2, AlertCircle, Clock, MapPin } from "lucide-react";
...
const PENDING_KEY = "streamer_application_pending";
...
export default function StreamerLanding() {
  ...
  const [isModalOpen, setIsModalOpen]   = useState(() => localStorage.getItem(PENDING_KEY) === "true");
  ...
  return (
    <div className="relative min-h-screen text-white pt-4 pb-20 overflow-hidden" style={{ backgroundColor: "#000" }}>
      ...
      <div className="absolute -top-20 right-0 w-[600px] h-[400px] rounded-full pointer-events-none" style={{ background: `radial-gradient(circle, rgba(85,222,232,0.15) 0%, transparent 70%)` }} />

      <div className="max-w-7xl mx-auto px-6 relative z-10">
        <div className="grid md:grid-cols-2 gap-10 md:gap-16 items-center min-h-[80vh]">
          <div>
            <div className="flex flex-wrap items-center gap-3 mb-4 md:mb-6">
              <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full border text-[10px] md:text-xs font-semibold uppercase tracking-widest" style={{ background: "rgba(85,222,232,0.08)", borderColor: "rgba(85,222,232,0.25)" }}>
              ...
              {(geoLoading || geoLabel) && (
                <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full border border-white/10 bg-white/5 text-[10px] md:text-xs font-bold uppercase tracking-widest text-[#55DEE8]">
                  <MapPin size={12} />
                  {geoLoading ? (
                    <span className="flex items-center gap-1.5 text-white/50">
                      Detecting... <span className="w-1.5 h-1.5 rounded-full bg-[#55DEE8] animate-pulse" />
                    </span>
                  ) : (
                    <span>{geoLabel}</span>
                  )}
            ...
            <button onClick={openModal} className="inline-flex items-center gap-3 font-bold text-black rounded-full px-8 py-4 hover:brightness-110 transition-all uppercase tracking-widest shadow-[0_0_20px_rgba(85,222,232,0.3)]" style={{ background: GRADIENT }}>
        ...
        <div className="mt-32 text-center">
          ...
          <div className="grid md:grid-cols-3 gap-8 max-w-5xl mx-auto">
            {[
              { icon: Video,       title: "Setup & Quality", desc: "List your equipment, streaming platforms, and broadcast quality capabilities." },
              { icon: CheckCircle, title: "Get Booked",      desc: "Organizers will book you for their tournaments. Manage your schedule in one place." },
              { icon: Globe,       title: "Go Live",         desc: "Stream the matches and get paid securely through the Kridaz platform." },
            ].map((s, i) => (
