  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-white">
        ...
        <button onClick={() => window.location.reload()} className="mt-4 text-[#B3DC26] text-xs uppercase font-bold border-b border-[#B3DC26]">Try Again</button>
      </div>
    );
  }
    ...
    <div className="h-full custom-scrollbar bg-[#000000] text-white w-full max-w-full overflow-x-hidden relative">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 xl:gap-6 relative z-10 border-b border-white/10 pb-6 w-full max-w-full mt-2 md:mt-0">
        <div className="min-w-0">
          ...
          <p className="text-white/70 font-inter text-[14px] md:text-[20px] mt-2 md:ml-4 leading-tight">Control your multisport facility roster.</p>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center py-32 bg-[#121212] border border-white/10 rounded-[16px] border-dashed shadow-[var(--shadow-2)]">
          ...
          <p className="text-[12px] font-normal text-white/70 uppercase tracking-[0.5px]">No active arenas found in your roster.</p>
          <Link to="/venue-owner/add-turf" className="mt-6 text-[#B3DC26] text-[11px] font-bold uppercase tracking-widest border-b border-[#B3DC26]/40 hover:border-[#B3DC26] transition-all pb-1">Initialize First Venue</Link>
        </div>
      )}
      </div>
    </div>
