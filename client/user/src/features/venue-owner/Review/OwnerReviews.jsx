    ...
    <div className="h-full custom-scrollbar bg-[#000000] text-white">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 relative z-10 border-b border-[#2D2D2D] pb-6">
        <div>
          ...
          <p className="text-[#878C9F] font-inter text-[20px] mt-2">Monitor player sentiment and service quality.</p>
        </div>
      </div>
          ...
          <div className="bg-[#000000] border border-[#2D2D2D] rounded-[8px] p-6 shadow-[var(--shadow-2)]">
             ...
             <ul className="space-y-3">
               {turfs.map((turf) => (
                 <li key={turf.id}>
                   <button
                     className={`w-full text-left p-4 rounded-[6px] border transition-all flex justify-between items-center group ${ selectedTurf === turf.id ? "bg-[#BFF367]/10 border-[#BFF367]/40 text-white" : "bg-[#1A1A1A] border-[#2D2D2D] text-[#878C9F] hover:border-[#BFF367]/20 hover:text-white" }`}
                     onClick={() => setSelectedTurf(turf.id)}
                   >
                     <span className="font-bold text-[13px] uppercase tracking-widest">{turf.name}</span>
                     <span className={`px-2 py-1 rounded-[4px] font-bold text-[11px] flex items-center gap-1.5 ${ selectedTurf === turf.id ? "bg-[#BFF367] text-black" : "bg-[#111] text-[#878C9F] border border-[#2D2D2D] group-hover:text-[#BFF367]" }`}>
            ...
            <>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div className="bg-[#000000] border border-[#2D2D2D] rounded-[8px] p-5 flex flex-col relative overflow-hidden shadow-[var(--shadow-2)] hover:border-[#BFF367]/30 transition-all duration-500">
                   ...
                   <p className="text-[12px] font-bold font-['Open_Sans'] text-[#878C9F] uppercase tracking-[0.5px] mb-1">Positive Sentiment</p>
                ...
                <div className="bg-[#000000] border border-[#2D2D2D] rounded-[8px] p-5 flex flex-col relative overflow-hidden shadow-[var(--shadow-2)] hover:border-[#BFF367]/30 transition-all duration-500">
                   ...
                   <p className="text-[12px] font-bold font-['Open_Sans'] text-[#878C9F] uppercase tracking-[0.5px] mb-1">Critical Reviews</p>
              ...
              <div className="bg-[#000000] border border-[#2D2D2D] rounded-[8px] p-6 space-y-6 shadow-[var(--shadow-2)]">
                ...
                <div className="space-y-4">
                  {reviews.length > 0 ? (
                    reviews.map((review) => (
                      <div key={review.id} className="bg-[#1A1A1A] border border-[#2D2D2D] p-5 rounded-[8px] hover:border-[#BFF367]/30 transition-colors group">
