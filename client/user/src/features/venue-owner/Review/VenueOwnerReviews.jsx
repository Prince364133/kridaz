    ...
    <div className="h-full custom-scrollbar bg-[#000000] text-white">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 relative z-10 border-b border-white/5 pb-6">
        <div>
          ...
          <p className="text-[#878C9F] font-inter font-light text-[14px] md:text-[20px] mt-2">Monitor player sentiment and service quality.</p>
        </div>
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        <div className="w-full lg:w-1/3 space-y-4 md:space-y-6">
          <div className="bg-[#121212] border border-white/5 rounded-[8px] p-4 md:p-6 shadow-[var(--shadow-2)]">
             <div className="flex items-center gap-2 mb-6">
                <div className="w-1 h-3 bg-[#BFF367] rounded-full" />
                <h3 className="text-[12px] font-bold font-['Open_Sans'] text-[#878C9F] uppercase tracking-[0.5px]">Select Arena</h3>
             </div>
             ...
             <ul className="space-y-3">
               {turfs.map((turf) => (
                 <li key={turf.id}>
                   <button
                     className={`w-full text-left p-4 rounded-[6px] border transition-all flex justify-between items-center group ${ selectedTurf === turf.id ? "bg-[#BFF367]/10 border-[#BFF367]/40 text-white" : "bg-[#1A1A1A] border-white/5 text-[#878C9F] hover:border-[#BFF367]/20 hover:text-white" }`}
                     onClick={() => setSelectedTurf(turf.id)}
                   >
                     <span className="font-bold text-[13px] uppercase tracking-widest">{turf.name}</span>
                     <span className={`px-2 py-1 rounded-[4px] font-bold text-[11px] flex items-center gap-1.5 ${ selectedTurf === turf.id ? "bg-[#BFF367] text-black" : "bg-[#111] text-[#878C9F] border border-white/5 group-hover:text-[#BFF367]" }`}>
        ...
        <div className="w-full lg:w-2/3 space-y-6">
          {currentTurf ? (
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-[#121212] border border-white/5 rounded-[8px] p-5 flex flex-col relative overflow-hidden shadow-[var(--shadow-2)] hover:border-[#BFF367]/30 transition-all duration-500">
                   <div className="absolute top-0 right-0 p-4 opacity-5 group-hover:opacity-10 transition-opacity">
                      <Star size={40} className="text-[#BFF367] fill-[#BFF367]" />
                   </div>
                   <p className="text-[12px] font-bold font-['Open_Sans'] text-[#878C9F] uppercase tracking-[0.5px] mb-1">Average Score</p>
                ...
                <div className="bg-[#121212] border border-white/5 rounded-[8px] p-5 flex flex-col relative overflow-hidden shadow-[var(--shadow-2)] hover:border-[#BFF367]/30 transition-all duration-500">
                   <div className="absolute top-0 right-0 p-4 opacity-5 group-hover:opacity-10 transition-opacity">
                      <Activity size={40} className="text-[#BFF367]" />
                   </div>
                   <p className="text-[12px] font-bold font-['Open_Sans'] text-[#878C9F] uppercase tracking-[0.5px] mb-1">Total Reviews</p>
                   <h3 className="text-2xl font-semibold text-[#BFF367] tracking-tight">{totalReviews}</h3>
                   <p className="text-[11px] text-[#444] mt-1 font-medium tracking-wide">All Time Ratings</p>
                </div>
              </div>

              {/* Review List */}
              <div className="bg-[#121212] border border-white/5 rounded-[8px] p-6 space-y-6 shadow-[var(--shadow-2)]">
                <div className="flex items-center justify-between border-b border-white/5 pb-4">
                  <h2 className="text-[13px] font-bold text-white uppercase tracking-widest flex items-center gap-2">
                     <span className="w-2 h-2 bg-[#BFF367] rounded-full"></span>
                ...
                <div className="space-y-4">
                  {reviews.length > 0 ? (
                    reviews.map((review) => (
                      <div key={review.id} className="bg-[#1A1A1A] border border-white/5 p-5 rounded-[8px] hover:border-[#BFF367]/30 transition-colors group">
                        <div className="flex justify-between items-start mb-4">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <h3 
                                onClick={() => review.user?._id && navigate(`/profile/${review.user._id}`)}
                                className="text-[13px] font-bold text-white uppercase tracking-widest cursor-pointer hover:text-[#BFF367] transition-colors"
                              >
                                {review.userName}
                              </h3>
                              <div className="flex items-center gap-2">
                                <p className="text-[10px] font-bold text-[#BFF367] uppercase tracking-widest">Verified Booking</p>
                                <span className="text-[8px] text-gray-500">GÇó</span>
                          ...
                          <div className="flex items-center gap-1 bg-[#111] px-2 py-1 rounded-[4px] border border-white/5">
                             {[...Array(5)].map((_, i) => (
                               <Star 
                                 key={i} 
                                 size={10} 
                                 className={`${i < review.rating ? "text-[#BFF367] fill-[#BFF367]" : "text-[#333]"}`} 
                               />
                             ))}
                          </div>
                        </div>
                        ...
                        <div className="flex justify-end border-t border-white/5 pt-3">
                           <button className="text-[10px] font-bold text-[#878C9F] uppercase tracking-widest hover:text-[#BFF367] flex items-center gap-1.5 transition-colors">
                    ...
                    <div className="flex flex-col items-center justify-center py-16 text-center border border-dashed border-white/5 rounded-[8px] bg-[#000000]">
                      <Activity size={32} className="text-[#333] mb-4" />
                      <p className="text-[12px] font-bold font-['Open_Sans'] text-[#878C9F] uppercase tracking-[0.5px]">
            ...
            <div className="h-full min-h-[250px] md:min-h-[400px] flex flex-col items-center justify-center border border-dashed border-white/5 rounded-[8px] bg-[#000000] shadow-[var(--shadow-2)] p-6 text-center">
              <Activity className="w-6 h-6 md:w-10 md:h-10 text-[#333] mb-4" />
              <p className="text-[10px] md:text-[12px] font-bold font-['Open_Sans'] text-[#878C9F] uppercase tracking-[0.5px]">
