          ...
          <div className="space-y-1">
            <div className="flex items-center gap-3">
              ...
              <h1 className="text-[28px] lg:text-[32px] font-bold font-['Open_Sans'] text-white tracking-tight leading-none uppercase">
                Student <span className="text-[#BFF367]">Roster</span>
              </h1>
            </div>
      ...
      {trainees.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {trainees.map((student) => (
            <div key={student._id} className="group relative bg-[#000000] border border-[#2D2D2D] rounded-[8px] p-6 hover:border-[#BFF367]/30 transition-all duration-500 shadow-[var(--shadow-2)]">
              <div className="flex items-center gap-4 mb-6">
                <div className="w-16 h-16 rounded-[6px] bg-[#BFF367]/10 border border-[#BFF367]/20 flex items-center justify-center overflow-hidden">
                  {student.avatar ? (
                    <img src={student.avatar} alt={student.name} className="w-full h-full object-cover" />
                  ) : (
                    <span className="text-2xl font-bold text-[#BFF367] font-inter">{student.name.charAt(0)}</span>
                  )}
                </div>
                <div>
                  <h3 className="text-lg font-bold text-white group-hover:text-[#BFF367] transition-colors font-inter tracking-tight">{student.name}</h3>
              ...
              <div className="space-y-3 mb-6">
                <div className="flex items-center gap-3 text-[13px] text-[#878C9F] font-inter">
                  <div className="w-8 h-8 rounded-[6px] bg-[#2D2D2D]/30 flex items-center justify-center border border-[#2D2D2D]">
                    <Mail size={14} className="text-[#BFF367]" />
                  </div>
                  {student.email}
                </div>
                <div className="flex items-center gap-3 text-[13px] text-[#878C9F] font-inter">
                  <div className="w-8 h-8 rounded-[6px] bg-[#2D2D2D]/30 flex items-center justify-center border border-[#2D2D2D]">
                    <Phone size={14} className="text-[#BFF367]" />
                  </div>
                  {student.phone}
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <button className="py-2.5 bg-[#BFF367]/10 hover:bg-[#BFF367]/20 text-[#BFF367] text-[10px] font-bold uppercase tracking-wider rounded-[6px] transition-all font-inter border border-[#BFF367]/20">
                  View Profile
                </button>
