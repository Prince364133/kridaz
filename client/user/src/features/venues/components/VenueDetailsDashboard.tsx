...
const VenueReviewSection = React.lazy(
  () => import("@/features/reviews").then((mod) => ({ default: mod.VenueReviewSection }))
);
...
export const VenueDetailsDashboard = () => {
  ...
  const successBookingId = query.bookingId;
  
  useEffect(() => {
    console.log("DEBUG PARAMS:", { successBookingId, query, windowUrl: window.location.href });
  }, [query, successBookingId]);
  ...
  const proceedToPayment = async (orderId: string, amount: number) => {
    ...
    try {
        const options = {
            ...
            amount: amount, // amount in paisa already from backend
            currency: "INR",
            name: "TurfSpot",
            ...
            handler: async (response: any) => {
                try {
                    ...
                    await BookingsApi.verifyPayment(verificationData);
  ...
  const handleConfirmPaymentSelection = async () => {
    if (bookingResponse) {
        ...
        setIsBooking(true);
        try {
            const paymentOrder = await BookingsApi.initiatePayment(
                bookingResponse.bookingId, 
                paymentType as any
            );
            if (paymentOrder?.orderId) {
                await proceedToPayment(paymentOrder.orderId, paymentOrder.amount);
            } else {
  ...
  useEffect(() => {
                       ...
                       try {
                           ...
                           const response = await BookingsApi.createBooking(bookingRequest);
                             if (response.allowPartialPayment && response.depositAmount) {
                                 ...
                                 await proceedToPayment(response.order.id, response.order.amount);
                             } else {
                                 const paymentOrder = await BookingsApi.initiatePayment(response.bookingId, 'FULL');
                                 if (paymentOrder?.orderId) {
                                     await proceedToPayment(paymentOrder.orderId, paymentOrder.amount);
                                 } else {
                                     throw new Error("Failed to initiate payment");
    ...
    }, [isAuthenticated, availableSlots, isBooking, params?.action, params?.slotId, navigate, venueId]);
        ...
        <DialogContent className="sm:max-w-lg">
          ...
          <div className="flex items-start space-x-2 py-4">
            ...
            <div className="grid gap-1.5 leading-none">
                ...
                >
                    I accept the <Link to="/terms" className="text-primary underline">Terms & Conditions</Link> and <Link to="/refund-policy" className="text-primary underline">Cancellation Policy</Link>
                </label>
            </div>
          </div>
