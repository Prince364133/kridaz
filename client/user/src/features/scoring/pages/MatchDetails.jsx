...
const MatchDetails = () => {
 ...
 <div className="flex items-center gap-2 text-sm font-bold text-gray-400">
 <MapPin size={16} className="text-primary" /> {game.ground?.name || game.customVenue || 'Self-Arranged'}
 </div>
 </div>
 </div>
 ...
 {isCompleted ? (
 <button 
 onClick={() => navigate(`/analytics/${game.shortId || game.id || game._id}`)}
 ...
 ) : game.invites?.filter(inv => inv.isProfessional && inv.role === 'UMPIRE').length > 0 ? (
 game.invites.filter(inv => inv.isProfessional && inv.role === 'UMPIRE').map(umpire => (
 <div 
 key={umpire.id}
 className="flex items-center gap-4 p-4 bg-white/5 rounded-2xl border border-white/10 transition-all group/u"
 >
 ...
 <div>
 <p className="text-sm font-black uppercase tracking-tight text-white">{umpire.name}</p>
 <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">Custom Umpire</p>
 </div>
 </div>
 ))
 ) : (
 ...
 <div>
 <p className="font-black uppercase tracking-tight">{game.customVenue}</p>
 <p className="text-xs text-gray-400 font-bold uppercase tracking-tight">Custom Venue</p>
 </div>
 </div>
 ) : (
