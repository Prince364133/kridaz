  ...
  return (
    <motion.div
      ...
      className={`relative overflow-hidden rounded-[20px] p-[1px] ${
        isLive ? 'bg-gradient-to-r from-red-500 via-[#55DEE8] to-[#BFF367]' : 'bg-white/10'
      }`}
    >
      <div className="relative bg-[#0F0F0F] rounded-[19px] p-5 h-full flex flex-col">
        {/* Header */}
        <div className="flex items-center justify-between mb-4">
          ...
          <div className={`px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest border ${
            isLive ? 'bg-red-500/10 text-red-500 border-red-500/30 animate-pulse' :
            isCompleted ? 'bg-white/5 text-white/40 border-white/10' :
            'bg-[#55DEE8]/10 text-[#55DEE8] border-[#55DEE8]/30'
          }`}>
            {isLive ? (
        ...
        {/* Teams */}
        <div className="flex items-center justify-between mb-5 bg-white/[0.02] p-4 rounded-xl border border-white/5 flex-1">
          <div className="flex flex-col items-center gap-2 flex-1">
            <div className="w-12 h-12 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center p-1">
              {match.teamA?.logo ? (
        ...
        <div className="grid grid-cols-2 gap-2 mt-auto">
          {isLive && match.youtubeStreamUrl ? (
            <button
              ...
              className="py-2.5 bg-red-500/10 border border-red-500/30 hover:bg-red-500/20 text-red-500 rounded-xl font-black uppercase tracking-widest text-[9px] flex items-center justify-center gap-1.5 transition-all"
            >
            ...
            <button
              ...
              className="py-2.5 bg-white/5 border border-white/10 hover:bg-white/10 text-white rounded-xl font-black uppercase tracking-widest text-[9px] flex items-center justify-center gap-1.5 transition-all"
            >
          ...
          <button
            ...
            className={`py-2.5 rounded-xl font-black uppercase tracking-widest text-[9px] flex items-center justify-center gap-1.5 transition-all ${
              isLive || isUpcoming 
                ? 'bg-gradient-to-r from-[#55DEE8] to-[#BFF367] text-black shadow-lg shadow-[#55DEE8]/20 hover:brightness-110' 
