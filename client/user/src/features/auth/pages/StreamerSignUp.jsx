...
const ACCENT = "#8B5CF6"; // Violet for streamers
...
const StreamerSignUp = () => {
  ...
  useEffect(() => {
    ...
    const streamerInvite = searchParams.get("streamerInvite");
    const tokenToUse = inviteToken || streamerInvite;
    ...
    if (tokenToUse) {
      localStorage.setItem("streamerInvite", tokenToUse);
      ...
      const fetchInviteDetails = async () => {
        try {
          ...
          if (res.data.success) {
            ...
            toast.success(`Welcome ${name || 'Streamer'}! Please complete your signup.`);
          }
        } catch (err) {
    ...
    // Redirect if already a streamer
    if (user?.role === "streamer" || role === "streamer") {
      navigate("/streamer");
    }
  }, [user, role, navigate]);
  ...
  return (
    <div className="min-h-screen bg-[#000] relative overflow-hidden flex flex-col items-center justify-start pt-24 lg:pt-32 pb-20 font-sans">
      <div className="absolute inset-0 z-0">
        <div className="absolute inset-0 bg-gradient-to-br from-black via-black/95 to-violet-900/10" />
        <div className="absolute top-1/3 right-0 w-[600px] h-[600px] bg-violet-900/10 blur-[150px] rounded-full pointer-events-none" />
      </div>
        ...
        <div className="hidden lg:flex lg:col-span-2 flex-col space-y-12">
          <div className="space-y-6">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-violet-500/20 bg-violet-500/5">
              ...
              <span className="text-[10px] font-bold tracking-[0.2em] text-violet-400 uppercase">Broadcasting Network</span>
            </div>
            ...
            <p className="text-sm text-white/40 uppercase tracking-widest max-w-sm leading-relaxed">
              Join the Kridaz elite broadcasting network. Stream professional matches and build your audience.
            </p>
          </div>

          <div className="grid grid-cols-1 gap-6">
            {[
              { icon: Camera, title: "High Quality", desc: "Broadcast matches in crystal clear quality" },
              { icon: Cast, title: "Reach", desc: "Grow your audience across platforms" },
              { icon: MonitorPlay, title: "Engagement", desc: "Provide live commentary and interactive coverage" }
            ].map((item, i) => (
              <div key={i} className="flex gap-4 group cursor-default">
                <div className="p-3 rounded-2xl bg-white/5 border border-white/10 group-hover:bg-violet-500/10 group-hover:border-violet-500/30 transition-all duration-500">
          ...
          <div className="bg-white/[0.03] backdrop-blur-xl border border-white/10 rounded-[40px] p-8 lg:p-12 shadow-2xl relative overflow-hidden">
            ...
            <div className="mb-8">
              <h2 className="text-3xl font-bold text-white mb-2 uppercase tracking-tight">
                {currentStep === 1 ? "Streamer Profile" : currentStep === 2 ? "Channel Details" : "Verification Documents"}
              </h2>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
              {currentStep === 1 && (
                <div className="grid md:grid-cols-2 gap-5">
                  <div className="md:col-span-2 relative group">
                    <User className="absolute left-5 top-1/2 -translate-y-1/2 text-gray-500 group-focus-within:text-violet-400 transition-colors" size={20} />
                    <input
                      {...register("name")}
                      placeholder="FULL NAME"
                      className="w-full bg-white/5 border border-white/10 focus:border-violet-500/50 rounded-2xl h-16 pl-14 pr-5 text-white outline-none transition-all uppercase text-xs font-bold tracking-widest"
                    />
                    {errors.name && <p className="text-red-500 text-[10px] mt-1 ml-2 uppercase font-bold">{errors.name.message}</p>}
                  </div>

                  <div className="md:col-span-2 relative group">
                    <UserSquare2 className="absolute left-5 top-1/2 -translate-y-1/2 text-gray-500 group-focus-within:text-violet-400 transition-colors" size={20} />
                    <input
                      {...register("username")}
                  ...
                  <div className="relative group">
                    <Mail className="absolute left-5 top-1/2 -translate-y-1/2 text-gray-500 group-focus-within:text-violet-400 transition-colors" size={20} />
                    <input
                      {...register("email")}
                      type="email"
                  ...
                  <div className="relative group">
                    <Phone className="absolute left-5 top-1/2 -translate-y-1/2 text-gray-500 group-focus-within:text-violet-400 transition-colors" size={20} />
                    <input
                      {...register("phone")}
                  ...
                  <div className="relative group">
                    ...
                    <select
                      {...register("gender")}
                      className="w-full bg-white/5 border border-white/10 focus:border-violet-500/50 rounded-2xl h-16 pl-14 pr-5 text-white outline-none transition-all uppercase text-xs font-bold tracking-widest appearance-none"
                    >
                  ...
                  <div className="relative group">
                    <MapPin className="absolute left-5 top-1/2 -translate-y-1/2 text-gray-500 group-focus-within:text-violet-400 transition-colors" size={20} />
                    <input
                      {...register("location")}
                      placeholder="LOCATION"
                      className="w-full bg-white/5 border border-white/10 focus:border-violet-500/50 rounded-2xl h-16 pl-14 pr-14 text-white outline-none transition-all uppercase text-xs font-bold tracking-widest"
                    />
                    <button
                      type="button"
                      onClick={fetchLocation}
                      disabled={isFetchingLocation}
                      className="absolute right-4 top-1/2 -translate-y-1/2 p-2 hover:bg-white/10 rounded-xl transition-all text-violet-400 disabled:opacity-50"
                    >
                  ...
                  <div className="relative group">
                    <Lock className="absolute left-5 top-1/2 -translate-y-1/2 text-gray-500 group-focus-within:text-violet-400 transition-colors" size={20} />
                    <input
                      {...register("password")}
                      type="password"
              ...
              {currentStep === 2 && (
                <div className="grid md:grid-cols-2 gap-5">
                  <div className="md:col-span-2 relative group">
                    <Building2 className="absolute left-5 top-1/2 -translate-y-1/2 text-gray-500 group-focus-within:text-violet-400 transition-colors" size={20} />
                    <input
                      {...register("businessDetails.businessName")}
                      placeholder="CHANNEL NAME / ORGANIZATION"
                      className="w-full bg-white/5 border border-white/10 focus:border-violet-500/50 rounded-2xl h-16 pl-14 pr-5 text-white outline-none transition-all uppercase text-xs font-bold tracking-widest"
                    />
                  ...
                  <div className="relative group">
                    <BarChart3 className="absolute left-5 top-1/2 -translate-y-1/2 text-gray-500 group-focus-within:text-violet-400 transition-colors" size={20} />
                    <input
                      {...register("businessDetails.experience")}
                      placeholder="YEARS OF STREAMING EXPERIENCE"
                      className="w-full bg-white/5 border border-white/10 focus:border-violet-500/50 rounded-2xl h-16 pl-14 pr-5 text-white outline-none transition-all uppercase text-xs font-bold tracking-widest"
                    />
                  ...
                  <div className="relative group">
                    <Video className="absolute left-5 top-1/2 -translate-y-1/2 text-gray-500 group-focus-within:text-violet-400 transition-colors" size={20} />
                    <input
                  ...
                  <div className="relative group">
                    <Locate className="absolute left-5 top-1/2 -translate-y-1/2 text-gray-500 group-focus-within:text-violet-400 transition-colors" size={20} />
                    <input
                  ...
                  <div className="relative group">
                    <Globe className="absolute left-5 top-1/2 -translate-y-1/2 text-gray-500 group-focus-within:text-violet-400 transition-colors" size={20} />
                    <input
                  ...
                  <div className="relative group">
                    <FileText className="absolute left-5 top-1/2 -translate-y-1/2 text-gray-500 group-focus-within:text-violet-400 transition-colors" size={20} />
                    <input
                      {...register("businessDetails.registrationNumber")}
                      placeholder="CHANNEL LINK / ID"
                      className="w-full bg-white/5 border border-white/10 focus:border-violet-500/50 rounded-2xl h-16 pl-14 pr-5 text-white outline-none transition-all uppercase text-xs font-bold tracking-widest"
                    />
              ...
              {currentStep === 3 && (
                <div className="grid md:grid-cols-2 gap-5">
                  <div className="md:col-span-2 p-6 rounded-2xl bg-violet-500/5 border border-violet-500/20 mb-4">
                    <p className="text-[10px] text-violet-400 uppercase font-bold tracking-[0.2em] mb-2">Streamer Verification</p>
                    <p className="text-xs text-gray-400 leading-relaxed">Verification is required to start taking bookings. Upload your channel proof, ID proof, and address proof.</p>
                  </div>

                  <FileUpload label="Channel Analytics/Proof" onUploadSuccess={(url) => handleDocUpload(url, "Channel Proof")} />
                  ...
                  <FileUpload label="Equipment Setup/Gear List" onUploadSuccess={(url) => handleDocUpload(url, "Equipment List")} />
              ...
              <div className="flex gap-4 pt-6">
                ...
                {currentStep < 3 ? (
                  <button
                    type="button"
                    onClick={nextStep}
                    className="flex-[2] h-16 rounded-2xl bg-violet-600 text-white font-bold uppercase tracking-widest text-xs hover:bg-violet-700 transition-all flex items-center justify-center gap-2 shadow-[0_10px_30px_rgba(139,92,246,0.3)]"
                  >
                    Continue
                  ...
                  <button
                    type="submit"
                    disabled={loading}
                    className="flex-[2] h-16 rounded-2xl bg-violet-600 text-white font-bold uppercase tracking-widest text-xs hover:bg-violet-700 transition-all flex items-center justify-center gap-2 shadow-[0_10px_30px_rgba(139,92,246,0.3)] disabled:opacity-50"
                  >
                    {loading ? "Registering..." : "Complete Streamer Signup"}
                    <ArrowRight size={16} />
                  </button>
                )}
              </div>
            ...
            <div className="mt-8 pt-8 border-t border-white/5 flex flex-col sm:flex-row items-center justify-between gap-6">
               <div className="flex items-center gap-3">
                  ...
                  <span className="text-[10px] text-gray-500 uppercase font-bold tracking-widest">Verified Streamers Only</span>
               </div>
               <div className="flex items-center gap-6">
                  ...
                  <Link to="/partners" className="text-[10px] text-violet-400 hover:text-violet-300 uppercase font-bold tracking-widest transition-colors">Not a streamer?</Link>
               </div>
            </div>
          </div>
        </div>
