...
import { Popover, PopoverContent, PopoverTrigger } from "@workspace/ui/components/popover"; // Our custom Popover component
import { Calendar } from "@workspace/ui/components/calendar"; // Our custom Calendar component
...
export function HostAGameForm({ setFormData }: HostAGameFormProps) {
  ...
  const onFormSubmit = async (data: GameFormValues) => {
    ...
    try {
      ...
      router.push(routes.games.hosted);
    } catch {
  ...
  const handleCancel = () => {
    ...
    setCreateGameSuccess(false); // Reset success state
    router.push(routes.explore); // Redirect to explore or dashboard
  };
