...
export function PlayerNavbar() {
  ...
  const handleVenueDashboardClick = async () => {
    try {
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/auth/generate-handoff-token`, {
            method: 'POST',
            headers: { 
                'Content-Type': 'application/json',
                // Add Authorization header if not handled by cookie proxy or interceptor
                // Assuming cookies are handled automatically with 'credentials: include'
            },
